...
public final class TPM_CutAndBurn extends Script
{
    ...
    public TPM_CutAndBurn( Extension ex )
	{
        super( ex );
    }
    ...
    public int main() 
	{
		...
		if ( getX() < 166 || getX() > 213 || getY() < 625 || getY() > 677 )
		{
			if ( reset_time == -1L )
		...
		if ( getInventoryCount( 14 ) > 0 )
		{
			...
			if ( fire[1] == getX() && fire[2] == getY() || move_time != -1L )
			{
		...
		if ( logs[0] != -1 && logs[1] == getX() && logs[2] == getY() )
		{
			int[] fire = getObjectById( 97 );
				  
			if ( fire[1] == getX() && fire[2] == getY() || move_time != -1L )
			{
				pickupItem(  logs[0] , logs[1] , logs[2] );
				
				return random( 1200 , 2400 );
			}
			else
			{
				int tinderbox = getInventoryIndex( 166 );
	...
	{
		System.out.println( "[TPM_CutAndBurn] This script requires you to be standing in the ardougne bank" );
	}
    ...
    public void paint() 
	{
		...
		y+=15;
		drawString("Fires: " + fires , x + 10, y, 1, white);
    }
    ...
    public void onServerMessage( String str ) 
	{
		...
		if ( str.contains( "light a fire here" ) ) 
