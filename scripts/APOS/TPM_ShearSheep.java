...
public final class TPM_ShearSheep extends Script
{
    private long bank_time;
    private long menu_time;
	private long move_time;
	
    public TPM_ShearSheep( Extension ex )
	{
        super( ex );
    }
    ...
    public int main() 
	{
		if ( isBanking() )
		{
			if ( getInventoryCount( 145 ) > 0 )
			{
				deposit( 145 , getInventoryCount( 145 ) );
				
				return random( 1000 , 1500 );
			}
			
			if ( getInventoryCount( 144 ) == 0 )
			{
				if ( bankCount( 144 ) == 0 )
				{
					System.out.println( "[TPM_BlueDrags] The script has stopped, out of shears..." );
        ...
        else if ( getInventoryCount() == 30 || getInventoryCount( 144 ) == 0 )
		{
			...
			else if ( nearArdougneNorthGate() )
			{
				walkTo( 580 + random( 0 , 1 ) , 574 + random( 0 , 1 ) );
			}
			else if ( nearArdougneNorthFarm() )
			{
				walkTo( 563 + random( 0 , 1 ) , 560 + random( 0 , 1 ) );
			}
			else
			{
				outOfBounds();
		...
		else
		{
			...
			{
				walkTo( 561 + random( 0 , 1 ) , 543 + random( 0 , 1 ) );
				
				return random( 2400 , 4800 );
			}
			else if ( nearArdougneNorthFarm() )
			{
				int[] gate = getObjectById( 60 );
				...
				{
					int   shear = getInventoryIndex( 144 );
					int[] sheep = getNpcById( 2 );
			  
					if ( shear != -1 && sheep[0] != -1 )
					{
						useOnNpc( sheep[0] , shear );
	...
	{
		System.out.println( "[TPM_ShearSheep] This script requires you to be standing in the ardougne bank" );
	}
	...
	{
		return ( getX() >= 562 && getX() <= 585 && getY() >= 554 && getY() <= 574 );
	}
	  
	private boolean nearArdougneNorthFarm()
	{  
		return ( getX() >= 540 && getX() <= 579 && getY() >= 526 && getY() <= 554 );
	}
